...
-- ============================================================================
-- AGRODATA - Aquaculture statistical summaries (migration 0012)
-- Additive: new table + policies, and re-creates resync to include it.
-- ============================================================================
...
-- ============================================================================
-- AGRODATA - Transactional statistics recompute (migration 0013)
-- Deletes + re-inserts all four sectors' summaries for a period in one
-- transaction, so a failed recompute can never leave summaries partially wiped.
-- ============================================================================
...
begin
  ...
  return jsonb_build_object(
    'crops', n_crops, 'livestock', n_livestock,
    'fisheries', n_fisheries, 'aquaculture', n_aquaculture
  );
end $$;
...
-- ============================================================================
-- AGRODATA - Reference catalogs: measurement units & aquatic species (0014)
-- Read: active OMA profile. Write: admin only. Includes seeds and extends the
-- sequence-resync helper (this is the final definition of the function).
-- ============================================================================
...
begin
  foreach t in array ref_tables loop
    ...
    execute format('create policy "read_%1$s" on public.%1$s for select using (public.has_active_profile());', t);
    execute format('create policy "admin_ins_%1$s" on public.%1$s for insert with check (public.is_admin());', t);
    execute format('create policy "admin_upd_%1$s" on public.%1$s for update using (public.is_admin());', t);
    execute format('create policy "admin_del_%1$s" on public.%1$s for delete using (public.is_admin());', t);
  end loop;
end $$;
